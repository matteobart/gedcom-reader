...
def marriage_before_death(family, people):
    ...
    if not wife.alive:
        if (married_date - wife.death).days > 0: raise Exception("Married Date should be before death date of wife in family:", family.id)
    if not husband.alive:
        if (married_date - husband.death).days > 0: raise Exception("Married Date should be before death date of husband in family:", family.id)
    return True
